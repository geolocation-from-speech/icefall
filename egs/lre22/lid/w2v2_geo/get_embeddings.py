...
def get_embeds(
    ...
    tgts = batch["supervisions"]["language"]
    feature = batch["inputs"].to(device)
    feature_lens = batch["features_lens"].to(device)
    with torch.set_grad_enabled(is_training):
        x = model.module.get_global_embeddings(feature, feature_lens) if isinstance(model, DDP) else model.get_global_embeddings(feature, feature_lens)
    return tgts, x
...
def run(rank, world_size, args):
    ...
    for c in cuts:
        if c.id not in dups:
            cuts_dedup.append(c)
        dups.add(c.id)
    cuts = CutSet.from_cuts(cuts_dedup) 
    valid_dl = lid.valid_dataloaders(cuts)
    ...
    if world_size > 1:
        np.save(f'{params.exp_dir}/embeds/tgts_{params.suffix}_{rank}.npy', np.array(tgts))
        ...
        np.save(f'{params.exp_dir}/embeds/tgts_{params.suffix}.npy', np.array(tgts))
