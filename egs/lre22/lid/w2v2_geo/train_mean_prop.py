from lid_datamodule import LIDDataModule
import argparse
import copy
import logging
...
from geolocation_model import Wav2Vec2Model
...
with open("lang2idx.json", 'r') as f:
    idx2lang = json.load(f)

lang2idx = {l: int(i) for i, l in idx2lang.items()}
...
def add_model_arguments(parser):
    ...
    parser.add_argument("--locations", type=str, default="locations.npy")
...
def get_model(params: AttributeDict) -> nn.Module:
    geolocation_model = Wav2Vec2Model(
        modelpath=params.modelpath,
        freeze_feat_extractor=params.freeze_feat_extractor,
    )
    if params.pretrained is not None and params.pretrained != "":
        mdl_dict = torch.load(params.pretrained, map_location='cpu')
        geolocation_model.load_state_dict(mdl_dict['model'])  
    model = LIDModel(
        geolocation_model, len(lang2idx), pooling_type=params.pooling_type
    )
    return model
...
def train_one_epoch(
    ...
    for batch_idx, batch in enumerate(train_dl): 
        ...
        if batch_idx % params.valid_interval == 0 and not params.print_diagnostics:
            ...
            print("---------------------------------------------------")
            print(f"Epoch {params.cur_epoch}, validation: {valid_info}")
            print("---------------------------------------------------")
            if tb_writer is not None:
...
def run(rank, world_size, args):
    ...
    radio_ssl = LIDDataModule(args)

    train_cuts = radio_ssl.train_cuts()
    valid_cuts = radio_ssl.valid_cuts()
...
def main():
    parser = get_parser()
    LIDDataModule.add_arguments(parser)
