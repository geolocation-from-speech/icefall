...
def run(rank, world_size, args):
    ...
    if world_size > 1:
        np.save(f'{params.exp_dir}/results/tgts_{params.suffix}_{rank}.npy', tgts.cpu().numpy())
        np.save(f'{params.exp_dir}/results/preds_{params.suffix}_{rank}.npy', preds.numpy())
    ...
    if params.average_over_segments:
        ...
        logging.info(f"Rank {rank} seg_averaged_distance: {avg_dist_.sum().item()}")
        logging.info(f"Rank {rank} seg_averaged_distance naive: {avg_dist.sum().item()}")
    else:
        if world_size > 1:
            with open(params.exp_dir / "results" / f"results_{params.suffix}_{rank}.txt", 'w') as f:
                print(valid_info, file=f)
