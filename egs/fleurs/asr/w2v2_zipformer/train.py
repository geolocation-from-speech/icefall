...
from asr_datamodule import FleursAsrDataModule
...
def compute_loss(
    ...
    with torch.set_grad_enabled(is_training):
        x, x_lens = model(feature, feature_lens)
        y = y.values
...
def run(rank, world_size, args):
    ...
    fleurs = FleursAsrDataModule(args)
    train_cuts = load_manifest_lazy("data/manifests/cuts_train_shuf.jsonl.gz")
    valid_cuts = load_manifest_lazy("data/manifests/cuts_dev_shuf.jsonl.gz")
    ...
    train_dl = fleurs.train_dataloaders(
        train_cuts, sampler_state_dict=sampler_state_dict
    )

    valid_dl = fleurs.valid_dataloaders(valid_cuts)
...
def main():
    parser = get_parser()
    FleursAsrDataModule.add_arguments(parser)
