...
def get_args():
    ...
    parser.add_argument(
        "--modified",
        type=str2bool,
        help="Whether or not to use the modified topo"
    )

    return parser.parse_args()


def compile_HLG(lang_dir: str, lm: str = "G_3_gram", modified: bool = False) -> k2.Fsa:
    """
    Args:
      lang_dir:
...
def main():
    ...
    if not args.modified:
        ...
        if (lang_dir / "HLG_modified.pt").is_file():
            logging.info(f"{lang_dir}/HLG_modified.pt already exists - skipping")
            return


    logging.info(f"Processing {lang_dir}")

    HLG = compile_HLG(lang_dir, args.lm, args.modified)
    logging.info(f"Saving HLG.pt to {lang_dir}")
    if not args.modified:
        torch.save(HLG.as_dict(), f"{lang_dir}/HLG.pt")
    else:
        torch.save(HLG.as_dict(), f"{lang_dir}/HLG_modified.pt")
