...
class SOTCTCGraphCompiler(object):
    """
    A graph compiler for SOT CTC decoding with token-level timing constraints.

    This class is designed to construct decoding graphs (FSAs) for use in alignment or training
    under the Serialized Output Training (SOT) CTC (SOT-CTC) framework. It uses a SentencePiece model to map
    ...
    def get_seqs_and_constraints(
        ...
        # Since we are doing segment-level SOT, we assign the same start time to all tokens from the same segment  
        token_start_times = {}
        pos_sym_to_sym = {}
        seqs = []
        for i, (s, o, l, spk) in enumerate(zip(self.sp.encode(c), offsets, lens, spks)):
            ...
            for j, token in enumerate(s):
                ...
                token_start_times[pos_sym] = o # This part (+ j*samples_per_token) is not needed for the segment-level SOT
        ...
        for ki in token_start_times:
            for kj in token_start_times:
                ...
                far_enough = abs(diff) > 0 # or ki[1] == kj[1] 
                if far_enough:
                    num_constraints += 1
                else:
                    num_ovlps += 1
