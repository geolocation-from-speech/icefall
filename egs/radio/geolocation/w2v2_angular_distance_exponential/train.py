...
def get_parser():
    ...
    parser.add_argument(
        ...
        type=float,
        default=math.pi/2,
        help="A constant used in the loss function computation",
    )
    ...
    parser.add_argument(
        ...
        type=str2bool,
        default=True,
        help="Whether or not to include squared term in the exponent in the loss",
    )
...
def compute_loss(
    ...
    with torch.set_grad_enabled(is_training):
        ...
        angle = angular_distance(y_polar, x_polar)
        angle_sum = angle.sum()
        if params.square_exp:
            loss = (math.pi * (1. - torch.exp(-(params.alpha*angle)**2))).sum()
        else:
            loss = (math.pi * (1. - torch.exp(-(params.alpha*angle)))).sum()
        #loss = (math.pi * (1. - torch.exp(-((math.pi/2)*angle)**2))).sum()
        info = MetricsTracker()
        ...
        info[f"avg_dist"] = 6378.1 * angle_sum.detach().cpu().item()
